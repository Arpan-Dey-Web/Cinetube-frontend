...
export default async function MovieDetails({
  ...
  const { id: routeSegments } = await params;
  const id = routeSegments.at(0) ?? "";
  ...
  const movie = await getMovieDetails(id, cookieHeader || undefined);
  if (!movie) {
    return notFound();
  }
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="container mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12 py-12 px-4 md:px-8">
        <div className="lg:col-span-2 space-y-16">
          <section className="rounded-[2rem] border border-border bg-card/20 p-6 md:p-8">
            <div className="grid gap-8 md:grid-cols-[1.2fr_0.8fr]">
              ...
              <div className="space-y-4 rounded-[1.5rem] border border-border bg-background/70 p-5">
